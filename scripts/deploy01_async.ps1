# === AI24X deploy01 ASYNC (long-task dispatch to brain 01 game, 42.192.1.93) ===
# Usage: powershell -File scripts\deploy01_async.ps1 <task-md>
# Sends task to 01 and triggers openclaw in BACKGROUND via runner (returns ~5-10s).
# Progress: ssh Administrator@42.192.1.93 "dir C:\Users\Administrator\ops\<task>.out.log"
param([Parameter(Mandatory=$true)][string]$TaskFile)

$ErrorActionPreference = "Stop"
$SshHost = "Administrator@42.192.1.93"
$RemoteOps = "C:\Users\Administrator\ops"
$TaskName = Split-Path $TaskFile -Leaf
$RemoteTask = "$RemoteOps\$TaskName"

$RunnerLocal = Join-Path $env:TEMP "run_async_01.ps1"
$RunnerRemote = "$RemoteOps\run_async_01.ps1"
...
Write-Host "==> scp task + runner -> 01" -ForegroundColor Cyan
...
Write-Host "==> trigger background run on 01..." -ForegroundColor Cyan
...
Write-Host "==> ASYNC dispatched OK. 01 running in background." -ForegroundColor Green
