...
async function solve(page) {
  const grid = await page.evaluate(() => [...document.querySelectorAll('.cell')].map(c => c.textContent || ''));
  const N = 7, dirs = [];
  for (let r=0;r<N;r++){ dirs.push([]); for(let c=0;c<N;c++) dirs[r][c] = ARROWS.indexOf(grid[r*N+c]); }
  const parent = {}, start='0,0', end=(N-1)+','+(N-1);
  const q=[{key:start,r:0,c:0}], visited=new Set([start]);
  let goal=null;
  while(q.length){
    const cur=q.shift();
  ...
  if(!goal) return null;
  const moves=[]; let k=goal.key;
  while(k!==start){ const p=parent[k]; moves.push({cell:p.from,rot:p.rot}); k=p.from; }
  moves.reverse();
...
(async()=>{
  ...
  const clickCell = async (r,c,times) => { for(let i=0;i<times;i++){ await page.mouse.click(box.x+c*cw+cw/2, box.y+r*ch+ch/2); await page.waitForTimeout(100); } };
  for (let lv=0; lv<5; lv++) {
    const sol = await solve(page);
    if (!sol) { console.log('L'+(lv+1)+': NO SOLUTION'); break; }
    for (const m of sol.moves) { const [r,c]=m.cell.split(',').map(Number); await clickCell(r,c,m.rot); }
    await page.click('#goBtn');
    await page.waitForTimeout(700);
    const st = await page.evaluate(() => ({ overlay: document.getElementById('overlay').classList.contains('show'), ovT: document.getElementById('ovT').textContent, stars: document.getElementById('stars').textContent, rot: document.getElementById('rotLine').textContent }));
