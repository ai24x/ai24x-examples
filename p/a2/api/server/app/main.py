...
from .auth import create_token, get_current_user_id, get_optional_user_id
...
from .signals import build_markers_v1, candles_from_tencent_like_pack
from .schemas import (
    AdminLoginIn,
    AdminOtpSendIn,
    InviteBindIn,
...
def _identity_post(path: str, json_body: dict, *, extra_headers: dict[str, str] | None = None) -> dict:
    ...
    if not base:
        raise HTTPException(status_code=503, detail="未配置主站身份 API（环境变量 AI24X_IDENTITY_API_BASE 或管理后台 identity_api_base）")
    url = f"{base}{path}"
...
def _identity_get(path: str) -> dict:
    ...
    if not base:
        raise HTTPException(
            status_code=503,
            detail="未配置主站身份 API（环境变量 AI24X_IDENTITY_API_BASE 或管理后台 identity_api_base）",
        )
    url = f"{base}{path}"
    headers: dict[str, str] = {}
    p = path.rstrip("/") or "/"
...
def admin_otp_send(request: Request, body: AdminOtpSendIn) -> dict:
    ...
    # 仅当「系统配置」关闭管理 OTP 时，才允许固定联调码（预演省事）。开启公网生产开关后必须走主站真实短信。
    ...
    if dc and not admin_browser_otp_feature_enabled():
        ...
        return {
            ...
            "ttl_s": ttl,
            "dev_code": dc,
            "message": "已使用 AI24X_ADMIN_OTP_DEV_CODE（未调用主站发短信；登录验证码请填该码）。开启「系统配置」管理 OTP 后将不再使用本码。",
        }
    ...
    if str(settings.env or "").lower() in ("prod", "production"):
        raise HTTPException(
            status_code=503,
            detail="生产环境须配置 AI24X_IDENTITY_API_BASE 以发送短信验证码",
        )
    ...
    return {
        ...
        "ttl_s": ttl,
        "dev_code": code,
        "message": "未配置主站身份 API：开发态返回 dev_code（勿用于生产）",
    }
...
def request_code(body: RequestCodeIn) -> RequestCodeOut:
    ...
    return RequestCodeOut(ok=True, dev_code=dev_code, message="未配置统一身份：开发态固定验证码")
...
def login(body: LoginIn) -> LoginOut:
    ...
    raise HTTPException(status_code=503, detail="生产环境须配置 AI24X_IDENTITY_API_BASE 并使用密码登录")
...
async def api_signals(
    ...
    try:
        ...
        markers = build_markers_v1(candles)
        return {"code": 0, "msg": "ok", "data": {"version": "a2-v1", "markers": markers}}
    except HTTPException:
        raise
    except Exception as e:
