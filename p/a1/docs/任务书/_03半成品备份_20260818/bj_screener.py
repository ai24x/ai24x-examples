...
DEFAULT_CFG: dict[str, Any] = {
    ...
    "aiMin": 60.0, "topN": 5, "cap": 42, "aiTop": 5, "aiOk": True,
...
def analyze(bars: list[list[Any]], cand: dict[str, Any], cfg: dict[str, Any], market: str = "bj") -> dict[str, Any]:
    ...
    # P1-2 B：量能门槛 1.2×→1.0×、位置 ≤50%→≤60%（放宽减少漏检，仍要求站稳 MA5）
    fr_idx = last - gc_days + 1
    fr_vol_ok = False
    if 0 < fr_idx < n:
        _fr_base = sum(v[max(0, fr_idx - 20):fr_idx]) / max(1, fr_idx)
        fr_vol_ok = _fr_base > 0 and v[fr_idx] >= 1.0 * _fr_base
    macd_first_red = bool(
        1 <= gc_days <= 3
        and fr_idx >= 1
        and hist[fr_idx - 1] <= 0
    ...
    for i in range(tries):
        try:
            payload = await fetch_tx_kline(
                _stock_secid(code), "day", count=170, timeout=8.0,
...
def pb_view(out: dict[str, Any]) -> dict[str, Any]:
    ...
    # 回踩企稳数据源拓宽（P1-3）：扫描结果不足时，用复盘样本（tags 含 首板/回踩 形态）补齐，
    # 零上游成本，保证今日必有内容；数据源标注（复盘样本 / 全市场扫描）随接口返回。
    try:
        _pb_all = list(o.get("picks") or []) + list(o.get("runners") or [])
        _have_codes = {str(x.get("code") or "") for x in _pb_all}
        _need = max(0, 12 - len(_pb_all))
        if _need > 0:
            from .daily_report import ARCHIVE_ROOT
            ...
            if not os.path.exists(_sp):
                _archs = sorted([x for x in os.listdir(ARCHIVE_ROOT)
                                ...
                                reverse=True)
                for _d2 in _archs:
                    _sp = os.path.join(ARCHIVE_ROOT, _d2, "sector_score.json")
                    if os.path.exists(_sp):
                        _d = _d2
                        break
            if os.path.exists(_sp):
                _kw = ("首板", "回踩", "涨停")
                _fill: list[dict[str, Any]] = []
                _sc = json.load(open(_sp, encoding="utf-8"))
                for _sec, _rows in (_sc or {}).items():
                    if not isinstance(_rows, list):
                        continue
                    for _r in _rows:
                        if not isinstance(_r, dict):
                            continue
                        _code = str(_r.get("code") or "")
                        if not _code or _code in _have_codes:
                            continue
                        _tags = [str(x) for x in (_r.get("tags") or [])]
                        if not any(any(_k in _t for _k in _kw) for _t in _tags):
                            continue
                        _have_codes.add(_code)
                        _fill.append({
                            "code": _code, "name": str(_r.get("name") or ""),
                            "score": _r.get("score"), "up_pct": _r.get("up_pct"),
                            "risks": [str(x) for x in (_r.get("risks") or [])][:3],
                            "src": "复盘样本", "board": str(_sec or ""),
                        })
                        if len(_fill) >= _need:
                            break
                _fill.sort(key=lambda x: -(x.get("score") or 0))
                o["picks"] = list(o.get("picks") or []) + _fill[:max(0, 8 - len(o.get("picks") or []))]
                o["runners"] = list(o.get("runners") or []) + _fill[8:][:max(0, 12 - len(_pb_all))]
    except Exception:
        pass
    for _x in (o.get("picks") or []) + (o.get("runners") or []):
        if isinstance(_x, dict) and not _x.get("src"):
            _x["src"] = "全市场扫描"
    return o
...
async def run_scan(
    ...
    if market in ("all", "hs", "kc") and _dml_obs and not boards_only:
        ...
        for _c in _obs_cands:
            ...
            if _code in _have2:
                ...
                cands.append(_c)
    # 全局补池最后：板块/主线/次主线成分优先占用 K线精筛名额
    cands.sort(key=lambda x: (1 if x.get("global") else 0, -(x.get("amount") or 0)))
    if len(cands) > int(cfg["cap"]):
        ...
        _trunc.extend(_ml_extra[: int(cfg.get("mainlineMemberCap") or 40) + int(cfg.get("obsMemberCap") or 30)])
        cands = _trunc
    ...
    if market in ("all", "hs", "kc") and _dml_names:
        try:
            ...
            # 主线真实资金批量补拉：不在资金热度池的主线（如煤炭）也能显示今日/5日主力
            ...
            for c in all_sorted:
                ...
                if c.get("mainHit"):
                    ...
                    if _f < (62 if defensive_mode else 58):
                        continue
                    if style_mode == "fan" and not (c.get("revHit") or c.get("mainHit") or c.get("obsHit")):
                        continue  # 电风扇：只做回踩/主线低吸，不追纯热度
