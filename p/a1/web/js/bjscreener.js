...
window.AI24X_BJScreener = (function () {
  ...
  function marketLabel(m) {
    ...
    if (m === "tight") return "大波段";
    return m === "all" ? "沪深京全市场" : "北证全市场";
  }
  function colMarket(d) {
    return (d && d.market_code) || state.market || "bj";
  }
  function isColNoMacd(m) {
    return ["tight", "low10", "pb", "mlpb", "breakout", "leader"].indexOf(m) >= 0;
  }
  function isHsKcCode6(code) {
    code = String(code || "").padStart(6, "0");
    if (/^(43|83|87|88|89|92)/.test(code)) return false;
    return /^(00|60|30|68)/.test(code);
  }
  function filterTightPicks(list) {
    return (list || []).filter(function (p) {
      if (!p || !isHsKcCode6(p.code)) return false;
      var pp = p.patterns || {};
      // 回放/专栏归档已按算法筛过：有 tightZt 必留；无 patterns 字段时仍允许沪深/创业/科创
      if (pp && Object.keys(pp).length) return !!pp.tightZt;
      return true;
    });
  }
  function hasPickChart(p) {
    var ch = p && p.chart;
  ...
  function marketScope(d) {
    ...
    if (m === "tight") return "大波段 · 多头粘合后再发散，且近期有过有效涨停（仅沪深/创业/科创）";
    return m === "all" ? "沪深京全市场" : "北证全市场";
  }
  ...
  function maTierBadge(p) {
    var t = String(p.maTier || (p.factors && p.factors.maTier) || "").toUpperCase();
    if (p.aboveMa144 || t === "S") return '<span class="tier-badge tier-ma-s" title="短均站上中长期均线">趋势更强</span>';
    if (p.ma60Strong) return '<span class="tier-badge tier-ma-a" title="短均明显高于中期均线且已持续">趋势良好</span>';
    if (p.aboveMa60 || t === "A") return '<span class="tier-badge tier-ma-a" title="短均站上中期均线">刚站上中期</span>';
    if (t === "B") return '<span class="tier-badge tier-ma-b" title="中期均线开始走好">启动初期</span>';
    return "";
  }
  function dualBadge(p) {
    if (p.dualConfirm || (p.decision && p.decision.dualConfirm)) {
      return '<span class="tier-badge tier-dual">热门强势·双确认</span>';
    }
    return "";
  }
  function decisionStrip(p) {
    var d = p.decision || {};
    if (!d.action && !d.invalid) return "";
    var lane = (d.laneLabel || (p.tier === "watch" ? "观察" : "精选"));
    var bits = [];
    bits.push('<span class="dec-lane">' + esc(lane) + '</span>');
    if (d.confirm) bits.push('<span class="dec-confirm">' + esc(d.confirm) + '</span>');
    if (d.ztNote) bits.push('<span class="dec-zt">' + esc(d.ztNote) + '</span>');
    if (d.trend) bits.push('<span class="dec-trend">' + esc(d.trend) + '</span>');
    return '<div class="pick-decision">' +
      '<div class="dec-row">' + bits.join("") + '</div>' +
      (d.action ? '<div class="dec-action">怎么看：' + esc(d.action) + '</div>' : '') +
      (d.invalid ? '<div class="dec-invalid">何时先放下：' + esc(d.invalid) + '</div>' : '') +
      '</div>';
  }
  function staleAlgoBanner(d, picks) {
    var list = picks || d.picks || [];
    if (!list.length) return "";
    var hasNew = list.some(function (p) {
  ...
  function tierBadge(p) {
    if (p.tier === "king") return '<span class="tier-badge tier-king">⭐ 今日精选</span>';
    if (p.tier === "key") return '<span class="tier-badge tier-key">精选</span>';
    if (p.tier === "watch") return '<span class="tier-badge tier-watch">观察</span>';
    return "";
  }
  function roleBadge(p) {
    ...
    if (p.pickRole === "hot_strong") return '<span class="tier-badge tier-hot">热门强势</span>';
    if (p.pickRole === "catchup") return '<span class="tier-badge tier-catchup">补涨观察</span>';
    if (p.pickRole === "watch") return '<span class="tier-badge tier-watch">跟踪观察</span>';
    return "";
  }
  function pickCardHtml(p, i) {
    var chgCls = cls(p.pct);
    var isKing = p.tier === "king";
    var isWatch = p.tier === "watch";
    var lv = p.levels || {};
    var st = p.strategy || {};
    var stLine = "";
    if (st && (st.entry || st.period)) {
      ...
      stLine = '<div class="pick-strategy"><span class="st-tag">技术参考</span>' +
        ...
        ' ｜ <span class="st-disc">不追高·缩量回踩·破位/长阴约8%先观望</span></div>';
    }
    return '<div class="pick-card' + (isKing ? " king" : (p.tier === "key" ? " key" : (isWatch ? " watch" : ""))) + '">' +
      '<div class="pick-head">' +
        '<div class="ph-l"><span class="pick-num">' + (p.rank != null ? p.rank : (i + 1)) + '</span> ' +
          ...
          tierBadge(p) + maTierBadge(p) + dualBadge(p) + roleBadge(p) + mainBadge(p) + indBadge(p) +
      ...
      coreTags(p) +
      decisionStrip(p) +
      '<div class="pick-sub">综合分 <b>' + (p.final != null ? p.final : "—") + '</b> ｜ 位置 <b>' + num((p.pos != null ? p.pos * 100 : 0), 0) + '%</b> ｜ 市值 ' + money(p.mcap) +
      ' ｜ 成交 ' + money(p.amount) + ' ｜ 5日 ' + pct(p.chg5) + ' ｜ 换手 ' + num(p.turnover, 1) + '%</div>' +
      (p.chart && (p.chart.closes || p.chart.c) ? '<div class="kbox"><canvas data-bj-code="' + esc(p.code) + '"></canvas></div>' : '<div class="kbox kbox-empty">暂无K线，可点名称查看行情</div>') +
      '<div class="levels">' +
        '<span class="s">支撑 ' + num(lv.s1, 2) + ' / ' + num(lv.s2, 2) + '</span>' +
        '<span class="p">压力 ' + num(lv.p1, 2) + ' / ' + num(lv.p2, 2) + '</span>' +
        '<span class="stop">破位参考 ' + num(lv.stop, 2) + '</span>' +
      '</div>' +
      stLine +
      fundHtml(p) +
      '<div class="pick-reason">为什么入选：' + esc(reasonOf(p)) + '</div>' +
      bearishHtml(p) +
      '</div>';
  }
  ...
  function reasonOf(p) {
    ...
    if (a.leaderOk) r.push("主线龙头·资金+趋势确认");
  ...
  function renderMainlines(d) {
    ...
    function avoidChipsHtml(names) {
      ...
      return '<div class="ml-obs-row">' + names.map(function (nm) {
        ...
        return '<span class="ml-avoid">' + esc(nm) + "</span>";
      }).join("") + "</div>";
    }
    function attackBodyHtml() {
      ...
      if (isFan || (finalized && obs.length)) {
        return '<span class="ml-none">暂无主线</span><span class="ml-hint">轮动快，先看观察</span>';
      }
      if (!finalized && !obs.length) {
        return '<span class="ml-none">主线生成中</span><span class="ml-hint">收盘后自动锁定</span>';
      }
      return '<span class="ml-none">暂无</span>';
    }

    var avoidNames = splitStrategyNames(_mdStrategy.avoid);
    box.innerHTML =
      '<div class="sec-label">今日主线</div>' +
      '<div class="ml-trio">' +
      '<div class="ml-card ml-card-attack"><div class="ml-kicker"><i class="ic-dot gold" aria-hidden="true"></i>主攻</div><div class="ml-card-body">' + attackBodyHtml() + "</div></div>" +
      '<div class="ml-card ml-card-obs"><div class="ml-kicker"><i class="ic-dot cool" aria-hidden="true"></i><span class="ic-emoji" aria-hidden="true">👀</span>观察</div><div class="ml-card-body">' + obsChipsHtml(obs) + "</div></div>" +
      '<div class="ml-card ml-card-avoid"><div class="ml-kicker"><i class="ic-dot rose" aria-hidden="true"></i>回避</div><div class="ml-card-body">' + avoidChipsHtml(avoidNames) + "</div></div>" +
      "</div>";
  }
  function renderBoardRank(d) {
    ...
    if (!box) return;
    // 算法专栏不展示全市场板块热度（北证代表会误当成栏目推荐）
    var mkt = colMarket(d);
    if (mkt === "tight" || mkt === "macd" || mkt === "low10") {
      box.innerHTML = "";
      return;
    }
  ...
  var _mdStrategy = { observe: "", avoid: "" };
  function splitStrategyNames(txt) {
    return String(txt || "").split(/[、,，;；/／|]/).map(function (s) { return String(s || "").trim(); }).filter(Boolean);
  }
  /** 观察板块：扫描 API + 简报 MD（原 bj-strategy 条）合并去重 */
  function resolveObserves(d) {
    var out = [];
    var seen = {};
    function add(nm, src, obj) {
      nm = String(nm || "").trim();
      if (!nm) return;
      var key = nm.replace(/\s+/g, "");
      if (seen[key]) return;
      seen[key] = true;
      out.push(obj && obj.name ? obj : { name: nm, src: src || "daily" });
    }
    ((d && d.observes) || []).forEach(function (o) {
      add((o && o.name) ? o.name : o, (o && o.src) || "daily", o);
    });
    splitStrategyNames(_mdStrategy.observe).forEach(function (nm) {
      add(nm, "report");
    });
    return out;
  }
  ...
  function renderColumnLedger(rows, track) {
    ...
    if (track && track.items) {
      track.items.forEach(function (t) {
        trackMap[(t.date || "") + "|" + (t.code || "")] = t;
      });
    }
    ...
    h += '<div class="pb-ledger-wrap"><table class="pb-ledger"><thead><tr>' +
      '<th>日期</th><th>代码</th><th>名称</th><th>形态</th><th>评分</th><th>入选价</th>' +
    ...
    rows.forEach(function (r) {
      ...
      var qHref = quoteHref({ code: r.code, name: r.name });
      var qTitle = "在AI行情官中查看 " + String(r.name || r.code || "");
      ...
      var st = tk && tk.state === "tracking" ? ' <span class="chip-sub">跟踪中</span>' : "";
      h += "<tr>" +
        '<td><a href="' + href + '" target="_blank" rel="noopener">' + esc(r.date || "") + "</a>" + st + "</td>" +
        '<td class="mono"><a href="' + qHref + '" target="_blank" rel="noopener" title="' + esc(qTitle) + '">' + esc(r.code || "") + "</a></td>" +
        '<td><a href="' + qHref + '" target="_blank" rel="noopener" title="' + esc(qTitle) + '"><b>' + esc(r.name || "") + "</b></a></td>" +
        '<td class="pb-pat">' + esc(r.pattern || "—") + "</td>" +
        "<td>" + (r.final != null ? esc(r.final) : "—") + "</td>" +
        "<td>" + (r.price != null ? esc(r.price) : "—") + "</td>" +
        "<td>" + fmtPct(tk && tk.fwd5) + "</td>" +
        "<td>" + fmtPct(tk && tk.fwd10) + "</td>" +
        "<td>" + rel5 + "</td>" +
        "<td>" + rel10 + "</td>" +
        "</tr>";
    });
  ...
  function historyRow(it, market) {
    var picks = (it.picks || []).map(function (p, pi) {
      ...
      var pat = p.pattern ? ('<span class="hr-pat">' + esc(p.pattern) + '</span>') : "";
  ...
  function renderPicks(d) {
    ...
    if (!picks.length && mkt === "tight" && d.replay_preview && d.replay_preview.picks) {
      ...
      if (rpicks.length) {
        picks = rpicks;
        replayMode = true;
        replayDateKey = d.replay_preview.date8 || d.replay_preview.date || "";
      }
    }
    ...
    if (!picks.length) {
      ...
      if (_watch0.length) {
        _emptyHtml += '<div class="sec-label">' + dataDayLabel(d) + '观察 · ' + _watch0.length + ' 只</div>';
        _emptyHtml += '<div class="bj-note">今日暂无精选；以下为观察池，适合跟踪，不建议当作重点。</div><div class="pick-grid watch-grid">';
        _watch0.forEach(function (p, i) { _emptyHtml += pickCardHtml(p, i); });
        _emptyHtml += '</div>';
      }
      box.innerHTML = _emptyHtml;
      if (_watch0.length) {
        requestAnimationFrame(function () {
          _watch0.forEach(function (p) {
    ...
    if (replayMode) {
      var _rd = d.replay_preview.date || replayDateKey || "";
      html += '<div class="bj-section">历史大波段参考 · 算法回放（数据日期 ' + esc(_rd) + '）</div>';
      html += '<div class="notice stale">今日暂无合格标的；以下为最近交易日按大波段算法回放的结果，供打开核对。不构成今日推荐。</div>';
      if (replayDateKey || _rd) {
        html += '<button class="btn-blue" style="margin:8px 0 12px" onclick="window.openReplay(\'' + esc(_rd || replayDateKey) + '\',\'tight\')">⚡ 用最新算法回放该日</button>';
      }
    } else if (d.archive) {
      ...
      html += staleAlgoBanner(d, picks);
      html += '<div class="sec-label">' + dataDayLabel(d) + '精选 · ' + nPicks + ' 只</div>';
      if (d.pickLaneNote) html += '<div class="bj-note">' + esc(d.pickLaneNote) + '</div>';
      if (d.mainline_gap) {
    ...
    box.innerHTML = html;
    requestAnimationFrame(function () {
      picks.concat(watch).forEach(function (p) {
        var cv = document.querySelector('canvas[data-bj-code="' + p.code + '"]');
        if (cv && p.chart) drawChart(cv, p.chart);
      });
    });
...
// MACD 副图：柱 + DIF/DEA + 首根红/绿加亮 + 金叉/死叉箭头
  ...
  function fetchCachedResult() {
    // 只读缓存；旧后端无 /result 路由时回退为「无缓存」，避免整页「加载失败：Not Found」
    return apiFetch("/api/bj/screener/result?market=" + encodeURIComponent(state.market)).catch(function (e) {
      var st = e && e.status;
      var msg = String((e && e.message) || "");
      if (st === 404 || /not\s*found/i.test(msg)) {
        return { ok: false, error: "no_cache", message: "暂无今日结果，请点「开始扫描」" };
      }
      throw e;
    });
  }
  ...
  function load(force) {
    ...
    fetchCachedResult().then(function (d) {
      if (seq !== reqSeq) return;
      if (d && (d.running === true || d.scanning === true)) {
        if (btn) btn.textContent = "扫描中…";
        statusLoading();
        waitDeadline = Date.now() + 12 * 60 * 1000;
        waitScanDone(seq, btn);
        return;
      }
  ...
  function applyScanResult(d, seq, btn) {
    ...
    if (d.stale) {
      ...
      setStatus(marketLabel(state.market) + "板块视图（" + fmtDate() + "）" + (d.cached ? " · 已载入" + dataDayLabel(d) + "结果" : ""));
    } else if (d.cached) {
      setStatus(marketLabel(state.market) + " · 已载入" + dataDayLabel(d) + "结果", false);
    } else {
      setStatus(marketLabel(state.market) + "扫描完成（" + fmtDate() + "）");
    }
  }
  ...
  function bindTabs() {
    ...
    for (var i = 0; i < tabs.length; i++) {
      tabs[i].addEventListener("click", function () {
        ...
        ["bj-result", "bj-history", "bj-boardrank", "bj-pb-ledger"].forEach(function (id) {
          var el = $(id); if (el) { el.innerHTML = ""; if (id === "bj-pb-ledger") el.hidden = true; }
        });
  ...
  function renderMarket(md, vip, asof, label, indexesBlob) {
    ...
    h += "</span></summary>";
    h += '<div class="mkt-signal compact"><div class="mkt-idx-wrap">';
    ...
    _mdStrategy.observe = observe;
    _mdStrategy.avoid = avoid;
    if (state.data) renderMainlines(state.data);
  ...
  function winrateRowHtml(p) {
    ...
    var qTitle = "在AI行情官中查看 " + String(p.name || p.code || "");
    return '<tr>' +
      '<td class="muted">' + esc(p.asof) + '</td>' +
      '<td class="win-name"><a class="win-link" href="' + quoteHref(p) + '" target="_blank" rel="noopener" title="' + esc(qTitle) + '">' + esc(p.name || p.code) + tier + '</a>' +
      '<a class="win-link muted" href="' + quoteHref(p) + '" target="_blank" rel="noopener" title="' + esc(qTitle) + '"> ' + esc(p.code) + '</a></td>' +
  ...
  var _RACE_MARKETS = ["pb", "mlpb", "breakout", "leader", "macd", "low10", "tight"];
  ...
  function boot() {
    ...
    if (ap.date) {
      ...
      if (ap.market === "bj" || ap.market === "all" || ap.market === "hs" || ap.market === "kc" || ap.market === "bj_all" || ap.market === "pb" || ap.market === "mlpb" || ap.market === "breakout" || ap.market === "leader" || ap.market === "low10" || ap.market === "tight") {
