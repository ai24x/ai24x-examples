...
3) 启动（本地推荐端口：静态 18003，API 18033）

```powershell
uvicorn app.main:app --host 127.0.0.1 --port 18033 --reload
```

打开：`http://127.0.0.1:18033/docs`

管理后台（浏览器）：默认 `http://127.0.0.1:18033/admin20260501/login`（工作台去掉 `/login`；路径由 `AI24X_ADMIN_MOUNT_PATH` 控制）。
