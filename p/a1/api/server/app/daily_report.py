...
def build_md(data, vip=True):
    ...
    A("")
    A("> ⚡ **今日速览**：大盘【%s】｜主攻【%s】｜观察【%s】｜总仓【%s】｜操作【主线回踩低吸、破位即撤】" % (
        mkt_lab,
        "、".join("**%s**" % x for x in main_lines) if main_lines else "今日无达标板块",
        "、".join(observes) if observes else "无",
        "≤3 成" if weak_flag else "≤5 成"))
    if weak_flag:
        A("> ⚠️ **环境警示**：大盘环境转弱（跌破MA20 / MACD绿柱），主线多为超跌反弹，总仓从严 ≤3 成，等重新站上 MA20 再进攻。")
    elif env.get("pts") is not None and float(env.get("pts")) >= 3:
        A("> ✅ **环境顺风**：大盘 MACD 翻红且站上 MA20，主线回踩企稳可低吸；注意不追高、破位即撤。")
    A("")
    A("---")
    A("")
    A("## 一、大盘信号验证")
    ...
    for ln in (full_md or "").split("\n"):
        ...
        if _h2("五、组合与风控"):
            ...
            if "s5" not in inserted:
                ...
                out.append("")
                out.append("> 仓位、止损与操作建议为 **VIP 专属**，开通 VIP 后自动解锁。")
                out.append("")
            continue
