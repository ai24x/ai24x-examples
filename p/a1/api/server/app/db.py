...
def invite_cfg_effective() -> dict[str, int]:
    ...
    return {
        ...
        "invite_reward_invitee_weekly": _cfg_int(
            "invite_reward_invitee_weekly", int(getattr(settings, "invite_reward_invitee_weekly", 0) or 0)
        ),
...
def init_db() -> None:
    with connect() as conn:
        if _is_pg():
            ...
            conn.execute(
                ...
                CREATE TABLE IF NOT EXISTS invite_activity (
                  ...
                  target_invites INTEGER NOT NULL DEFAULT 3,
                  require_activated INTEGER NOT NULL DEFAULT 1,
                  reward_plan TEXT NOT NULL DEFAULT 'vip_trial_99',
                  reward_days INTEGER NOT NULL DEFAULT 7,
                  reward_weekly INTEGER NOT NULL DEFAULT 0,
                  reward_daily INTEGER NOT NULL DEFAULT 0,
                  stack_enabled INTEGER NOT NULL DEFAULT 1,
                  stack_cap_days INTEGER NOT NULL DEFAULT 90,
                  created_at BIGINT NOT NULL,
                  updated_at BIGINT NOT NULL
                );
                """
            ...
            conn.execute(
                ...
                CREATE TABLE IF NOT EXISTS activity_rewards (
                  ...
                  created_at BIGINT NOT NULL,
                  UNIQUE(activity_id, user_id, milestone)
                );
                """
            )
        ...
        if _is_pg():
            conn.execute(
                """
                CREATE TABLE IF NOT EXISTS city_partner_applications (
                  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                  user_id BIGINT NOT NULL,
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_cpa_user ON city_partner_applications(user_id);")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_cpa_status ON city_partner_applications(status);")
        else:
            conn.execute(
                """
                CREATE TABLE IF NOT EXISTS city_partner_applications (
        ...
        # Partner contract attachments (user materials + official signed agreements).
        if _is_pg():
            conn.execute(
                """
                CREATE TABLE IF NOT EXISTS partner_contract_files (
                  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                  application_id BIGINT NOT NULL,
                  user_id BIGINT NOT NULL,
                  side TEXT NOT NULL DEFAULT 'user',      -- user / admin
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_pcf_app ON partner_contract_files(application_id);")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_pcf_user ON partner_contract_files(user_id);")
        else:
            conn.execute(
                """
                CREATE TABLE IF NOT EXISTS partner_contract_files (
        ...
        try:
            if _is_pg():
                ...
                # 裂变活动：叠加赠送 + 里程碑记录（老库补列；唯一约束改为按里程碑）
                conn.execute("ALTER TABLE invite_activity ADD COLUMN IF NOT EXISTS stack_enabled INTEGER NOT NULL DEFAULT 1;")
                conn.execute("ALTER TABLE invite_activity ADD COLUMN IF NOT EXISTS stack_cap_days INTEGER NOT NULL DEFAULT 90;")
                conn.execute("ALTER TABLE activity_rewards ADD COLUMN IF NOT EXISTS milestone INTEGER NOT NULL DEFAULT 1;")
                try:
                    conn.execute("ALTER TABLE activity_rewards DROP CONSTRAINT IF EXISTS activity_rewards_activity_id_user_id_key;")
                except Exception:
                    pass
                try:
                    conn.execute(
                        "CREATE UNIQUE INDEX IF NOT EXISTS uq_activity_rewards_milestone ON activity_rewards(activity_id, user_id, milestone);"
                    )
                except Exception:
                    pass
            else:
                ...
                # 裂变活动：叠加赠送 + 里程碑记录（老库补列）
                try:
                    acols = [r["name"] for r in conn.execute("PRAGMA table_info(invite_activity)").fetchall()]
                    if "stack_enabled" not in acols:
                        conn.execute("ALTER TABLE invite_activity ADD COLUMN stack_enabled INTEGER NOT NULL DEFAULT 1")
                    if "stack_cap_days" not in acols:
                        conn.execute("ALTER TABLE invite_activity ADD COLUMN stack_cap_days INTEGER NOT NULL DEFAULT 90")
                except Exception:
                    pass
                try:
                    rcols = [r["name"] for r in conn.execute("PRAGMA table_info(activity_rewards)").fetchall()]
                    if "milestone" not in rcols:
                        conn.execute("ALTER TABLE activity_rewards ADD COLUMN milestone INTEGER NOT NULL DEFAULT 1")
                except Exception:
                    pass
                try:
                    conn.execute(
...
def admin_feedback_reply(
    ...
    *,
    ...
    now = int(time.time())
    with connect() as conn:
        row = conn.execute("SELECT id, user_id, admin_reply FROM user_feedback WHERE id=?", (fid,)).fetchone()
        if not row:
        ...
        try:
            _notice_insert(
                conn,
                title="工单已回复" if st == "replied" else "工单已关闭",
                body="您的反馈工单 #%d 已%s：%s" % (fid, "回复" if st == "replied" else "关闭", (final_reply or "（无备注）")[:200]),
...
def consume_quota(user_id: int, secid: str, period: str, idempotency_key: str, ok: bool) -> dict:
    ...
    with connect() as conn:
        ...
        _activity_inviter: int | None = None
        ...
        _consume_result = {"deduped": False, "result": "ok", "consumed_at": now, "quota": get_quota_status(user_id)}

    # 裂变活动触发（事务外，确保能读到刚提交的 quota_ledger）：被邀请人首次有效查询后，
    # 检查邀请人是否达到活动门槛，达标则自动发放 VIP（幂等，失败不影响扣次结果）
    if _activity_inviter is not None:
        try:
            grant_invite_activity_reward(_activity_inviter)
        except Exception:
            pass
...
def _user_has_paid_partner_plan(user_id: int) -> bool:
    """是否已支付成长档/合作伙伴（城市合伙人申请前置条件）。"""
    uid = int(user_id)
    with connect() as conn:
        row = conn.execute(
            "SELECT 1 FROM pay_orders WHERE user_id=? AND plan IN ('agent_growth','agent_pro') AND status='paid' LIMIT 1",
            (uid,),
        ).fetchone()
        return bool(row)


def create_city_partner_application(*, user_id: int, region: str = "", contact_name: str = "", id_no: str = "", phone: str = "", note: str = "") -> dict[str, Any]:
    """前台提交城市合伙人申请（支付后人工审核流程）。"""
    uid = int(user_id)
    if uid <= 0:
        raise ValueError("invalid_user_id")
    region = str(region or "").strip()[:64]
    contact_name = str(contact_name or "").strip()[:64]
    id_no = str(id_no or "").strip()[:64]
    phone = str(phone or "").strip()[:32]
    note = str(note or "").strip()[:500]
    if not region or not contact_name:
        raise ValueError("region_and_contact_required")
    now = int(time.time())
    if agent_city_partner_info(uid).get("city_partner"):
        raise ValueError("already_city_partner")
    with connect() as conn:
        row = conn.execute(
            "SELECT id FROM city_partner_applications WHERE user_id=? AND status='pending' LIMIT 1",
            (uid,),
        ).fetchone()
        if row:
            raise ValueError("already_pending")
        if not _user_has_paid_partner_plan(uid):
            raise ValueError("need_paid_partner_plan")
        conn.execute(
            "INSERT INTO city_partner_applications(user_id, region, contact_name, id_no, phone, note, status, admin_note, created_at, reviewed_at) VALUES (?,?,?,?,?,?,'pending','',?,0)",
            (uid, region, contact_name, id_no, phone, note, now),
        )
        got = conn.execute(
            "SELECT id FROM city_partner_applications WHERE user_id=? ORDER BY id DESC LIMIT 1",
            (uid,),
        ).fetchone()
        app_id = int(got["id"]) if got else 0
    return {"ok": True, "application_id": app_id, "status": "pending"}


def get_city_partner_application_by_user(user_id: int) -> dict[str, Any] | None:
    """查询用户最近一条城市合伙人申请。"""
    uid = int(user_id)
    with connect() as conn:
        row = conn.execute(
            "SELECT id, user_id, region, contact_name, id_no, phone, note, status, admin_note, created_at, reviewed_at FROM city_partner_applications WHERE user_id=? ORDER BY id DESC LIMIT 1",
            (uid,),
        ).fetchone()
        if not row:
            return None
        return {
            ...
            "id_no": str(_row_get(row, "id_no") or ""),
            "phone": str(_row_get(row, "phone") or ""),
            "note": str(_row_get(row, "note") or ""),
            "status": str(row["status"] or ""),
            "admin_note": str(_row_get(row, "admin_note") or ""),
            "created_at": int(row["created_at"] or 0),
            "reviewed_at": int(_row_get(row, "reviewed_at") or 0),
        }


def get_city_partner_application_by_id(application_id: int) -> dict[str, Any] | None:
    """按申请 ID 查询（后台合同归档用）。"""
    ...
    with connect() as conn:
        row = conn.execute(
            "SELECT id, user_id, region, contact_name, id_no, phone, note, status, admin_note, created_at, reviewed_at FROM city_partner_applications WHERE id=?",
            (app_id,),
        ).fetchone()
        if not row:
            return None
...
def admin_list_city_partner_applications(*, status: str = "", q: str = "", limit: int = 50, offset: int = 0) -> dict[str, Any]:
    """后台列出城市合伙人申请（默认待审核优先）。"""
    ...
    off = max(0, int(offset))
    sql = """
        SELECT a.id, a.user_id, a.region, a.contact_name, a.phone, a.note, a.status, a.admin_note, a.created_at, a.reviewed_at,
               u.phone AS user_phone, u.email AS user_email
        FROM city_partner_applications a LEFT JOIN users u ON u.id = a.user_id
    """
    params: list = []
    conds = []
    ...
    if q:
        conds.append("(u.phone LIKE ? OR u.email LIKE ? OR CAST(a.user_id AS TEXT) LIKE ? OR a.region LIKE ? OR a.contact_name LIKE ?)")
        like = "%" + q + "%"
        params += [like, like, like, like, like]
    if conds:
        sql += " WHERE " + " AND ".join(conds)
    sql += " ORDER BY CASE a.status WHEN 'pending' THEN 0 ELSE 1 END, a.created_at DESC LIMIT ? OFFSET ?"
    params += [lim, off]
    with connect() as conn:
        rows = conn.execute(sql, tuple(params)).fetchall()
        items = []
        for r in rows or []:
            items.append({
                ...
                "note": str(_row_get(r, "note") or ""),
                "status": str(r["status"] or ""),
                "admin_note": str(_row_get(r, "admin_note") or ""),
                "created_at": int(r["created_at"] or 0),
                "reviewed_at": int(_row_get(r, "reviewed_at") or 0),
                "user_phone": _mask_identity(str(_row_get(r, "user_phone") or "")),
                "user_email": _mask_identity(str(_row_get(r, "user_email") or "")),
            })
        try:
            crow = conn.execute("SELECT COUNT(1) AS c FROM city_partner_applications" + (" WHERE " + " AND ".join(conds) if conds else ""), tuple(params[:-2])).fetchone()
            cnt = int(crow["c"] or 0) if crow else len(items)
        except Exception:
            cnt = len(items)
        return {"ok": True, "items": items, "total": cnt, "limit": lim, "offset": off}


def admin_review_city_partner_application(*, application_id: int, approve: bool, admin_note: str = "", region_override: str = "", agreement_no: str = "") -> dict[str, Any]:
    """后台审核城市合伙人申请：通过则签约（区域/协议编号），驳回则记录原因。"""
    app_id = int(application_id)
    if app_id <= 0:
        raise ValueError("invalid_application_id")
    approve = bool(approve)
    note = str(admin_note or "").strip()[:300]
    with connect() as conn:
        row = conn.execute("SELECT user_id, region, contact_name FROM city_partner_applications WHERE id=?", (app_id,)).fetchone()
        if not row:
            raise ValueError("application_not_found")
        uid = int(row["user_id"])
        region = str(region_override or "").strip()[:64] or str(row["region"] or "").strip()[:64]
        now = int(time.time())
        ag = ""
        if approve:
            ag = str(agreement_no or "").strip()[:64]
            if not ag:
                ag = "CP-%d-%04d" % (int(time.strftime("%Y", time.localtime(now))), app_id)
            admin_set_city_partner(user_id=uid, city_partner=True, city_region=region, city_agreement_no=ag)
        conn.execute(
            "UPDATE city_partner_applications SET status=?, admin_note=?, reviewed_at=? WHERE id=?",
            ("approved" if approve else "rejected", note, now, app_id),
        )
        try:
            _notice_insert(
                conn,
                title="城市合伙人申请已通过" if approve else "城市合伙人申请未通过",
                body=(
                    "恭喜！您申请的「%s」区域城市合伙人已通过审核并签约，协议编号 %s。正式合同归档后可在「我的-城市合伙人」下载查看。"
                    % (region, ag)
                    if approve
                    else "很遗憾，您的城市合伙人申请未通过：%s。可修正后重新申请。" % (note or "未填写原因")
                ),
                target_user_id=uid,
            )
        except Exception:
            pass
    return {"ok": True, "application_id": app_id, "approved": approve, "region": region, "agreement_no": ag}


def partner_contract_upload_dir() -> str:
    """合同/材料附件存储目录（api/server/data/contracts），不存在则创建。"""
    base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    d = os.path.join(base, "data", "contracts")
    try:
...
def _notice_insert(conn: Any, *, title: str, body: str, target_user_id: int) -> None:
    """同一连接内插入站内定向通知（scope=user）。"""
    now = int(time.time())
    conn.execute(
        """
        ...
        ) VALUES (?, ?, 'user', ?, '', 0, 'active', ?, ?, 'system', ?, ?)
        """,
        (str(title or "")[:120], str(body or ""), int(target_user_id), None, None, now, now),
    )


def _contract_file_row_dict(row: Any) -> dict[str, Any]:
    return {
        "id": int(row["id"]),
        ...
        "stored_name": str(_row_get(row, "stored_name") or ""),
        "size_bytes": int(_row_get(row, "size_bytes") or 0),
...
def save_partner_contract_file(*, application_id: int, user_id: int, side: str = "user", kind: str = "other", original_name: str = "", stored_name: str = "", size_bytes: int = 0, mime: str = "", note: str = "") -> dict[str, Any]:
    """写入合同/材料附件记录；admin 侧上传正式合同后自动站内通知用户。"""
    app_id = int(application_id)
    uid = int(user_id)
    side = str(side or "user").strip().lower()
    if side not in ("user", "admin"):
        side = "user"
    kind = str(kind or "other").strip()[:32] or "other"
    oname = str(original_name or "").strip()[:255]
    sname = str(stored_name or "").strip()[:128]
    mime = str(mime or "").strip()[:128]
    note = str(note or "").strip()[:300]
    size = max(0, int(size_bytes or 0))
    if app_id <= 0 or uid <= 0 or not sname:
        raise ValueError("invalid_contract_file")
    now = int(time.time())
    with connect() as conn:
        conn.execute(
            "INSERT INTO partner_contract_files(application_id, user_id, side, kind, original_name, stored_name, size_bytes, mime, note, created_at) VALUES (?,?,?,?,?,?,?,?,?,?)",
            (app_id, uid, side, kind, oname, sname, size, mime, note, now),
        )
        row = conn.execute("SELECT id FROM partner_contract_files ORDER BY id DESC LIMIT 1").fetchone()
        ...
        if side == "admin":
            try:
                _notice_insert(
                    conn,
                    title="城市合伙人合同已归档",
                    body="您的《城市合伙人合作协议》合同文件已归档（%s），可登录后在「我的-城市合伙人」下载查看。" % (oname or "正式合同"),
                    target_user_id=uid,
                )
            except Exception:
                pass
    return {"ok": True, "file_id": fid, "stored_name": sname}


def list_partner_contract_files(*, application_id: int = 0, user_id: int = 0, side: str = "") -> list[dict[str, Any]]:
    """附件列表（最近优先）。"""
    ...
    if side:
        conds.append("side = ?")
        params.append(side)
    sql = "SELECT * FROM partner_contract_files"
    if conds:
        sql += " WHERE " + " AND ".join(conds)
    sql += " ORDER BY created_at DESC, id DESC LIMIT 200"
    with connect() as conn:
        rows = conn.execute(sql, tuple(params)).fetchall() or []
        return [_contract_file_row_dict(r) for r in rows]


def get_partner_contract_file(file_id: int) -> dict[str, Any] | None:
    fid = int(file_id)
    with connect() as conn:
        row = conn.execute("SELECT * FROM partner_contract_files WHERE id=?", (fid,)).fetchone()
        if not row:
            return None
        return _contract_file_row_dict(row)


def delete_partner_contract_file(file_id: int, *, user_id: int = 0, is_admin: bool = False) -> dict[str, Any]:
    """删除附件记录并返回 stored_name（供调用方清理磁盘文件）。用户仅可删自己的 user 侧附件。"""
    fid = int(file_id)
    uid = int(user_id or 0)
    with connect() as conn:
        row = conn.execute("SELECT stored_name, user_id, side FROM partner_contract_files WHERE id=?", (fid,)).fetchone()
        if not row:
            raise ValueError("file_not_found")
        if not is_admin and (uid <= 0 or int(row["user_id"]) != uid or str(row["side"] or "").lower() != "user"):
            raise ValueError("no_permission")
        conn.execute("DELETE FROM partner_contract_files WHERE id=?", (fid,))
    return {"ok": True, "file_id": fid, "stored_name": str(row["stored_name"] or "")}
...
# ============ 邀请裂变活动（邀 N 人激活自动升 VIP，后台可配） ============

_ACTIVITY_PLAN_ORDER = {"free": 0, "vip_trial_99": 1, "vip_month": 2, "vip_year_999": 3}


def _activity_row_to_dict(r: Any) -> dict:
    return {
        "id": int(r["id"]),
        "enabled": int(r["enabled"] or 0),
        "name": str(r["name"] or ""),
        "description": str(r["description"] or ""),
        "start_at": int(r["start_at"] or 0),
        "end_at": int(r["end_at"] or 0),
        "target_invites": int(r["target_invites"] or 3),
        "require_activated": int(r["require_activated"] or 1),
        "reward_plan": str(r["reward_plan"] or "vip_trial_99"),
        "reward_days": int(r["reward_days"] or 7),
        "reward_weekly": int(r["reward_weekly"] or 0),
        "reward_daily": int(r["reward_daily"] or 0),
        "stack_enabled": int(r["stack_enabled"] or 0),
        "stack_cap_days": int(r["stack_cap_days"] or 0),
        "created_at": int(r["created_at"] or 0),
        "updated_at": int(r["updated_at"] or 0),
    }


def _activity_flag(v: Any) -> int:
...
def invite_activity_effective(now_ts: int | None = None) -> dict | None:
    """返回当前生效的裂变活动（enabled=1 且在起止时间内），无则 None。"""
    now = int(time.time()) if now_ts is None else int(now_ts)
    with connect() as conn:
        rows = conn.execute(
            "SELECT * FROM invite_activity WHERE enabled=1 ORDER BY id DESC LIMIT 20"
        ).fetchall()
    for r in rows:
        d = _activity_row_to_dict(r)
        if d["start_at"] > 0 and now < d["start_at"]:
            continue
        if d["end_at"] > 0 and now > d["end_at"]:
            continue
        return d
    return None


def invite_activity_get_all() -> list[dict]:
    out: list[dict] = []
    with connect() as conn:
        rows = conn.execute("SELECT * FROM invite_activity ORDER BY id DESC").fetchall()
        for r in rows:
            d = _activity_row_to_dict(r)
            rr = conn.execute(
                "SELECT COUNT(1) AS c FROM activity_rewards WHERE activity_id=?",
                (d["id"],),
            ).fetchone()
            d["rewards_count"] = int(rr["c"] or 0) if rr else 0
            out.append(d)
    return out


def invite_activity_get_by_id(activity_id: int) -> dict | None:
    with connect() as conn:
        row = conn.execute(
            "SELECT * FROM invite_activity WHERE id=?", (int(activity_id),)
        ).fetchone()
    if not row:
        return None
...
def invite_activity_save(cfg: dict) -> dict:
    """新增（id 缺省/0）或更新活动配置（未传字段保留现值）；返回保存后的活动行。"""
    act_id = int(cfg.get("id") or 0)
    now = int(time.time())
    with connect() as conn:
        if act_id:
            existing = conn.execute(
                "SELECT * FROM invite_activity WHERE id=?", (act_id,)
            ).fetchone()
            if not existing:
                raise ValueError("activity not found")
            cur = _activity_row_to_dict(existing)
        else:
            cur = {
                "enabled": 0,
                "name": "",
        ...
        enabled = _activity_flag(_pick("enabled", cur["enabled"]))
        name = str(_pick("name", cur["name"]) or "").strip()[:60]
        description = str(_pick("description", cur["description"]) or "").strip()[:300]
        start_at = _activity_ts(_pick("start_at", cur["start_at"]))
        end_at = _activity_ts(_pick("end_at", cur["end_at"]))
        target = max(1, min(int(_pick("target_invites", cur["target_invites"]) or 3), 1000))
        require_activated = _activity_flag(_pick("require_activated", cur["require_activated"]))
        plan = str(_pick("reward_plan", cur["reward_plan"]) or "vip_trial_99").strip().lower()
        if plan not in ("free", "vip_trial_99", "vip_month", "vip_year_999"):
            plan = "vip_trial_99"
        days = max(1, min(int(_pick("reward_days", cur["reward_days"]) or 7), 3650))
        weekly = max(0, int(_pick("reward_weekly", cur["reward_weekly"]) or 0))
        daily = max(0, int(_pick("reward_daily", cur["reward_daily"]) or 0))
        stack_enabled = _activity_flag(_pick("stack_enabled", cur["stack_enabled"]))
        stack_cap_days = max(0, min(int(_pick("stack_cap_days", cur["stack_cap_days"]) or 0), 3650))

        if act_id:
            conn.execute(
                """
                UPDATE invite_activity
                SET enabled=?, name=?, description=?, start_at=?, end_at=?, target_invites=?,
                    require_activated=?, reward_plan=?, reward_days=?, reward_weekly=?,
                    reward_daily=?, stack_enabled=?, stack_cap_days=?, updated_at=?
                WHERE id=?
                """,
                (
                    enabled, name, description, start_at, end_at, target,
                    require_activated, plan, days, weekly, daily, stack_enabled, stack_cap_days, now, act_id,
                ),
            )
        else:
            if _is_pg():
                row = conn.execute(
                    ...
                    INSERT INTO invite_activity(
                      enabled, name, description, start_at, end_at, target_invites,
                      require_activated, reward_plan, reward_days, reward_weekly,
                      reward_daily, stack_enabled, stack_cap_days, created_at, updated_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    RETURNING id
                    """,
                    (
                        enabled, name, description, start_at, end_at, target,
                        require_activated, plan, days, weekly, daily, stack_enabled, stack_cap_days, now, now,
                    ),
                ).fetchone()
                act_id = int(row["id"])
            else:
    ...
    return invite_activity_get_by_id(act_id) or {}


def _inviter_activated_count_in_conn(conn: Any, inviter_id: int, activity: dict) -> int:
    """统计邀请人在活动窗口内、满足激活条件（如配置）的有效被邀请人数。"""
    params: list[Any] = [int(inviter_id)]
    where = ""
    if _activity_flag(activity.get("require_activated")):
        where += (
            " AND EXISTS (SELECT 1 FROM quota_ledger q"
            " WHERE q.user_id=r.invitee_id AND q.result='ok' LIMIT 1)"
        )
    start_at = int(activity.get("start_at") or 0)
    end_at = int(activity.get("end_at") or 0)
    if start_at > 0:
    ...
    row = conn.execute(
        "SELECT COUNT(1) AS c FROM invite_relations r WHERE r.inviter_id=?" + where,
        tuple(params),
    ).fetchone()
    return int(row["c"] or 0) if row else 0


def invite_activity_progress_for_user(user_id: int, activity: dict) -> dict:
    with connect() as conn:
        activated = _inviter_activated_count_in_conn(conn, int(user_id), activity)
        rows = conn.execute(
            "SELECT milestone, days FROM activity_rewards WHERE activity_id=? AND user_id=?",
            (int(activity["id"]), int(user_id)),
        ).fetchall()
    target = max(1, int(activity.get("target_invites") or 3))
    days = max(1, int(activity.get("reward_days") or 7))
    stack_enabled = _activity_flag(activity.get("stack_enabled", 1))
    stack_cap_days = max(0, int(activity.get("stack_cap_days") or 0))
    rewarded_ms = len(rows)
    granted_days_sum = int(sum(int(r["days"] or 0) for r in rows))
    if activated >= target:
        milestone = max(1, int(activated // target))
    ...
    if stack_enabled:
        max_ms = None
        if stack_cap_days > 0:
            max_ms = max(1, stack_cap_days // days)
            if milestone > 0:
                milestone = min(milestone, max_ms)
        total_days = days * milestone
        if stack_cap_days > 0:
            total_days = min(total_days, stack_cap_days)
        capped = max_ms is not None and milestone >= max_ms
        next_target = milestone * target if capped else (milestone + 1) * target
    else:
        total_days = days if activated >= target else 0
        next_target = target
    return {
        "activated": activated,
        "target": target,
        "milestone": milestone,
        "done": activated >= target,
        "rewarded": rewarded_ms > 0,
        "rewarded_milestones": rewarded_ms,
        "granted_days": granted_days_sum,
        "total_days": total_days,
        "next_target": next_target,
        "stack_enabled": stack_enabled,
        "stack_cap_days": stack_cap_days,
    }


def grant_invite_activity_reward(inviter_id: int, activity: dict | None = None) -> dict:
    """
    裂变活动达标发放（里程碑叠加版）：
    每满 target 位激活好友进一档（第1档=target 人，第2档=2*target 人……），
    每档插入一条 activity_rewards(唯一约束 (activity_id,user_id,milestone) 防重)，
    VIP 按新到账档数顺延 days*档数（受 stack_cap_days 上限约束）。
    stack_enabled=0 时维持旧行为：只发第 1 档（target 人）。
    幂等：同一活动同一用户同一档只发一次；异常整体回滚。
    """
    inviter_id = int(inviter_id)
    if activity is None:
        activity = invite_activity_effective()
    if not activity:
        return {"granted": False, "reason": "no_active_activity"}
    act_id = int(activity["id"])
    target = max(1, int(activity.get("target_invites") or 3))
    plan = str(activity.get("reward_plan") or "vip_trial_99").strip().lower()
    days = max(1, int(activity.get("reward_days") or 7))
    weekly = max(0, int(activity.get("reward_weekly") or 0))
    daily = max(0, int(activity.get("reward_daily") or 0))
    stack_enabled = _activity_flag(activity.get("stack_enabled", 1))
    stack_cap_days = max(0, int(activity.get("stack_cap_days") or 0))
    now = int(time.time())
    ...
    with connect() as conn:
        # 1) 达标校验（事务内读最新）
        activated = _inviter_activated_count_in_conn(conn, inviter_id, activity)
        if activated < target:
            return {"granted": False, "reason": "below_target"}

        # 2) 计算里程碑：每满 target 人进一档；关闭叠加则只发第 1 档
        milestone = max(1, int(activated // target))
        if not stack_enabled:
            milestone = 1

        # 3) 已发放的里程碑（幂等判重 + 叠加上限核算）
        rows = conn.execute(
            "SELECT milestone, days FROM activity_rewards WHERE activity_id=? AND user_id=?",
            (act_id, inviter_id),
        ).fetchall()
        granted_ms_set = {int(r["milestone"] or 1) for r in rows}
        granted_days_sum = int(sum(int(r["days"] or 0) for r in rows))
        existing_cnt = len(granted_ms_set)
        if milestone <= existing_cnt:
            return {"granted": False, "reason": "already_rewarded"}
        new_ms = milestone - existing_cnt
        if stack_enabled and stack_cap_days > 0:
            allowed_by_cap = max(0, (stack_cap_days - granted_days_sum) // days)
            new_ms = min(new_ms, allowed_by_cap)
        if new_ms <= 0:
            return {"granted": False, "reason": "cap_reached"}

        # 4) 插入新里程碑记录（同事务内唯一约束防重）
        for i in range(existing_cnt + 1, existing_cnt + new_ms + 1):
            if _is_pg():
                conn.execute(
                    """
                    INSERT INTO activity_rewards(
                      activity_id, user_id, plan, days, weekly_quota, daily_quota,
                      activated_count, milestone, note, created_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, '', ?)
                    ON CONFLICT (activity_id, user_id, milestone) DO NOTHING
                    """,
                    (act_id, inviter_id, plan, days, weekly, daily, activated, i, now),
                )
            else:
                conn.execute(
                    """
        ...
        extend_days = days * new_ms

        # 5) 计算目标套餐（可配额外周/日额度；0=用套餐默认）
        np_, wk, dy, _ = _plan_defaults(plan)
        if weekly > 0:
            wk = int(weekly)
        if daily > 0:
            dy = int(daily)

        row = conn.execute("SELECT * FROM quota WHERE user_id=?", (inviter_id,)).fetchone()
        if not row:
            fp, fw, fd, _ = _plan_defaults("free")
            conn.execute(
                """
                INSERT INTO quota(
                  user_id, plan, monthly_limit, daily_limit, monthly_used, daily_used,
                  month_key, day_key, expires_at, updated_at
                ) VALUES (?, ?, ?, ?, 0, 0, ?, ?, NULL, ?)
                """,
                (inviter_id, fp, fw, fd, _week_key(now), _day_key(now), now),
            )
            row = conn.execute("SELECT * FROM quota WHERE user_id=?", (inviter_id,)).fetchone()
        ...
        cur_exp_ts = int(cur_exp) if cur_exp is not None else None

        before = {
            "plan": cur_plan,
            "weekly_limit": cur_weekly_limit,
            "daily_limit": cur_daily_limit,
            "weekly_used": cur_weekly_used,
            "daily_used": cur_daily_used,
            "expires_at": cur_exp_ts,
        }

        # 6) 顺延逻辑：起点 = max(now, 当前到期)；已持有更高档套餐且未到期时保留套餐额度、仅顺延
        start_ts = max(now, cur_exp_ts or now)
        new_exp = start_ts + extend_days * 86400
        cur_rank = _ACTIVITY_PLAN_ORDER.get(cur_plan, -1)
        new_rank = _ACTIVITY_PLAN_ORDER.get(np_, 0)
        if cur_rank > new_rank and cur_exp_ts is not None and cur_exp_ts > now:
            final_plan = cur_plan
            final_weekly = cur_weekly_limit
            final_daily = cur_daily_limit
        else:
            final_plan = np_
            final_weekly = wk
            final_daily = dy
        new_weekly_used = max(0, min(cur_weekly_used, final_weekly)) if final_weekly >= 0 else cur_weekly_used
        new_daily_used = max(0, min(cur_daily_used, final_daily)) if final_daily >= 0 else cur_daily_used

        conn.execute(
            """
            UPDATE quota
            SET plan=?, monthly_limit=?, daily_limit=?, monthly_used=?, daily_used=?,
                expires_at=?, updated_at=?
            WHERE user_id=?
            """,
            (final_plan, final_weekly, final_daily, new_weekly_used, new_daily_used, new_exp, now, inviter_id),
        )

        after = {
            "plan": final_plan,
            "weekly_limit": final_weekly,
            "daily_limit": final_daily,
            "weekly_used": new_weekly_used,
            "daily_used": new_daily_used,
            "expires_at": new_exp,
        }
        note = (
            f"invite_activity#{act_id}: activated={activated}/{target} "
            f"milestone={existing_cnt + 1}-{existing_cnt + new_ms} reward={plan}/{extend_days}d"
            + (f" cap={stack_cap_days}d" if stack_enabled and stack_cap_days > 0 else "")
        )
        conn.execute(
            """
            INSERT INTO admin_ops_ledger(user_id, actor, action, before_json, after_json, note, created_at)
    ...
    return {
        ...
        "activated": activated,
        "target": target,
        "milestone": existing_cnt + new_ms,
        "extend_days": extend_days,
        "stack_enabled": stack_enabled,
    }


def backfill_invite_activity_rewards(limit: int = 200, activity: dict | None = None) -> dict:
    """
    后台补扫：对当前生效活动，找出达标但未发放的邀请人并逐人发放。
    返回 {scanned, granted, already, failed}。
    """
    if activity is None:
        activity = invite_activity_effective()
    if not activity:
        return {"scanned": 0, "granted": 0, "already": 0, "failed": 0, "reason": "no_active_activity"}
    target = max(1, int(activity.get("target_invites") or 3))
    ...
    with connect() as conn:
        rows = conn.execute(
            "SELECT r.inviter_id, COUNT(1) AS c FROM invite_relations r WHERE 1=1"
            + where
            + " GROUP BY r.inviter_id HAVING COUNT(1) >= ? ORDER BY c DESC LIMIT ?",
            tuple(params),
        ).fetchall()
    candidates = [(int(r["inviter_id"]), int(r["c"])) for r in rows]
    granted = 0
    already = 0
    failed = 0
    for inviter_id, cnt in candidates:
        try:
            out = grant_invite_activity_reward(inviter_id, activity)
            if out.get("granted"):
                granted += 1
            else:
                already += 1
            ...
            failed += 1
    return {"scanned": len(candidates), "granted": granted, "already": already, "failed": failed}


def activity_rewards_list(limit: int = 100) -> list[dict]:
    lim = max(1, min(int(limit or 100), 500))
    with connect() as conn:
        rows = conn.execute(
            """
            SELECT ar.*, u.phone AS user_phone, u.email AS user_email
        ...
        out.append(
            {
                ...
                "weekly_quota": int(r["weekly_quota"] or 0),
                "daily_quota": int(r["daily_quota"] or 0),
                "activated_count": int(r["activated_count"] or 0),
                "note": str(r["note"] or ""),
                "created_at": int(r["created_at"] or 0),
