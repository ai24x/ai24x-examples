    ...
    <script>
      ...
      document.getElementById('btnSendOtp').addEventListener('click', async function(){
        ...
        try{
          ...
          if(j && j.dev_code){ err.textContent = '开发态验证码：'+j.dev_code+'（勿用于生产）'; }
        ...
        <div class="main-scroll">
            ...
            <div class="card" id="sec-wechat">
              ...
              <div class="field-row">
                ...
                <div class="field"><span class="lbl">跳过通知验签</span>
                  <select id="wx_wechat_notify_skip_verify" title="仅非 prod 建议；生产务必关">
          ...
          <section class="panel-page" id="p-hotspots-test">
            <div class="card" id="sec-hotspots-test">
              ...
              <div class="card" style="margin-top:12px;">
                <div class="row">
                  <span class="pill">跌幅榜调试（按涨跌幅 ths_daily）</span>
                  <span class="muted small">与右侧“每日跌幅榜 10→1”同口径（仅用于对齐核验）</span>
                  <button type="button" id="btnLosersPctDebug">调试</button>
                </div>
    ...
    <script>
      ...
      async function runLosersPctDebug(){
        ...
        try{
          ...
          setStatus('跌幅调试失败：'+(e && e.message ? e.message : 'unknown'));
        }
      }
      ...
      async function boot(){
        ...
        if($('btnLosersPctDebug')) $('btnLosersPctDebug').addEventListener('click', async function(){
          try{ await runLosersPctDebug(); }catch(e){ setStatus('跌幅调试失败：'+e.message); }
        });
