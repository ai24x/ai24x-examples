...
def list_hero_ops_alerts() -> list[dict[str, Any]]:
    ...
    for cid, row in risks.items():
        ...
        if now - float(row.get("ts") or 0) > 86400:
            continue
        # 切后仍健康（毛利 ≥ 黄线且未倒挂）→ 不进 P0/P1
        try:
            gm_after = float(row["gm_after"]) if row.get("gm_after") is not None else None
        except (TypeError, ValueError):
            gm_after = None
        stored_lvl = str(row.get("level") or "")
        if stored_lvl != "alarm" and (gm_after is None or gm_after >= warn_gm()):
            continue
        rid = "".join(c if c.isalnum() else "_" for c in str(cid))[:40]
        lvl = "error" if stored_lvl == "alarm" else "warn"
        title = str(row.get("title") or cid)
