# 山海渔（ShanHai Fisher）— 子项目 `p/game/fisher`
...
- 本目录：`p/game/fisher/`，与 `p/a/`（AI 行情官）同级，遵循 `p/README.md` 的「一项目一目录」约定。
...
见仓库根 `p/game/fisher/.gitignore`（含 `project.private.config.json` 等）。
