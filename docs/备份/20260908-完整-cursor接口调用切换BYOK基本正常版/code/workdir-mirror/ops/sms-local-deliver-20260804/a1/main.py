...
def proxy_sms_send(body: SmsSendProxyIn, request: Request) -> dict:
    """短信发送：sms_active_provider=local 时本地直发 106 网关（2026-08-04 雷总拍板独立）；否则转发主站。"""
    cfg = resolve_identity()
    ...
    if (cfg.sms_active_provider or "identity_proxy").strip().lower() in _local_providers:
        ...
        if not ok:
            raise HTTPException(status_code=503, detail=f"短信服务暂不可用：{msg}")
...
def register(body: RegisterIn) -> LoginOut:
    ...
    if body.phone:
        ...
        if local_sms:
            # 本地直发模式下：本地校验验证码，通过后带内部密钥让主站跳过短信校验
            ...
            extra = {"X-SMS-Internal-Key": (cfg.sms_internal_key or "").strip()}
    else:
