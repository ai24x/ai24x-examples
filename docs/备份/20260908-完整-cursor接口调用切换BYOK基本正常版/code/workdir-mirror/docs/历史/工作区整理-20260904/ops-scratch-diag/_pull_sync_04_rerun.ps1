...
try {
  ...
  Write-Host ("open_status=" + [int]$r.StatusCode)
...
try {
  ...
  if ($_.Exception.Response) { Write-Host ("open_models=" + [int]$_.Exception.Response.StatusCode) } else { Write-Host $_.Exception.Message }
}
Write-Host "=== dirty prod paths ==="
git status --porcelain -- api web/js web/console.html p/open/api p/open/web p/markets/api p/markets/web | ForEach-Object { Write-Host $_ }
